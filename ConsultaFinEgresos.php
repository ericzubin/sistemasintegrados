...
echo("<table border=1 class='table table-striped table-bordered table-hover'>");		echo("<tr> <td> Id </td> <td> Fecha </td> <td> Monto </td> <td> Status </td> <td> IdSuministro </td></tr>");
		for($a=0; $a<mysqli_num_rows($Consulta); $a++){
			...
			echo("<td> $fila[3]</td>");
			echo("<td> $fila[4]</td>");
			echo("<td> <a class='btn btn-primary' href='ActualizaFINEgresos.php?id=$fila[0]&Fecha=$fila[1]&Monto=$fila[2]&Status=$fila[3]&IdSuministro=$fila[4]'>Actualizar</a> </td>");
			echo("<td> <a class='btn btn-danger' href='EliminaFINEgresos.php?id=$fila[0]'>Eliminar</a> </td>");
			echo("</tr>");
		}
	echo("</table>");
echo ("</div>
