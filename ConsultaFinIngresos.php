...
<div align="center">
                    <h1 class="page-header">Consultar Ingreso</h1>
</div>
<div class="col-lg-2">
...
<?php
  if(isset($_GET['Idg']))
  {
    ...
    $Con=Conectar();
    $Querydel="DELETE FROM finingresos  where IdIngreso =".$_GET['Idg'];
    mysqli_query($Con,$Querydel) or die("error");
...
$Campo=$_POST['Campo'];
$Query="SELECT * FROM finingresos  where $Campo = '$Criterio'";
...
echo("<tr>  <td>Idingreso</td>  <td>fecha</td>  <td>Monto</td>  <td>status </td> <td> </td> <td> </td> </tr>");
...
for($a=0; $a < mysqli_num_rows($Consulta) ; $a++)
	...
	echo ("<td> $fila[2] </td>");
	echo ("<td> $fila[3] </td>");
  echo ("<td> <a class='btn btn-primary' href='ActualizarFinIngresos.php?IdIngreso=".$fila[0]."&Fecha=".$fila[1]."&Monto=".$fila[2]."&Status=".$fila[3]."'> Actualizar</a></td>");

	echo ("<td>  <a class='btn btn-danger' href='ConsultaFinIngresos.php?Idg=".$fila[0]."'>Eliminar</a></td>");
