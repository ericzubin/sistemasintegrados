...
<title>Consultar Cuentas Por Pagar</title>
...
<div align="center">
                    <h1 class="page-header">Consultar Cuentas Por Pagar</h1>
</div>
<div class="col-lg-2">
...
Fecha</label>
    ...
    <input type="submit" class="btn btn-default" name="Submit" value="Consultar">    </label>
  </p>
  <p>&nbsp;</p>
</form>
<?php
...
echo("<table border=1 class='table table-striped table-bordered table-hover'>");
		...
		for($a=0; $a<mysqli_num_rows($Consulta); $a++){
			...
			echo("<td> $fila[5]</td>");
			echo("<td> $fila[6]</td>");
			echo("<td> <a class='btn btn-primary' href='ActualizaFINCuentasPorPagar.php?id=$fila[0]&Fecha=$fila[1]&Monto=$fila[2]&Status=$fila[3]&Plazo=$fila[4]&Tasa=$fila[5]&IdCompra=$fila[6]'>Actualizar</a> </td>");
			echo("<td> <a class='btn btn-danger' href='EliminaFINCuentasPorPagar.php?id=$fila[0]'>Eliminar</a> </td>");
			echo("</tr>");
		}
		echo("</table>");
echo ("</div>
